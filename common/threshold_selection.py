    ...
    for threshold in thresholds:
        ...
        for event, outcome in rows:
            if event.signal_score >= threshold:
                by_day[event.observed_at.date()].append((event.signal_score, outcome.directional_net_return))
        selected = [item for values in by_day.values() for item in sorted(values, reverse=True)[:daily_alert_limit]]
        if selected:
            candidates.append(ThresholdSelection(threshold, sum(value for _, value in selected) / len(selected), len(selected)))
    if not candidates:
