...
class NotificationFormatter:
    ...
    def _format_attention_queue(
        ...
        if config.ATTENTION_VISIBLE_MODEL in {
            ...
            for candidate in sorted(
                ...
                lines.extend(
                    self._format_attention_candidate(candidate, reverse_sector_map)
                )
            folded = [candidate for candidate in candidates if not candidate.displayed]
            if folded:
                symbols = ", ".join(
                    candidate.market.removeprefix("KRW-") for candidate in folded
                )
                lines.append(f"\n📁 **추가 필터 통과 ({len(folded)}개)**")
                lines.append(f"- {symbols}")
            return "\n".join(lines)

        lines = [f"🎯 **관심종목 큐 (주요 {len(visible)} / 전체 {len(candidates)})**"]
        for lane in [
            AttentionLane.FOCUS,
            AttentionLane.EARLY,
            ...
            lane_visible = [
                candidate for candidate in visible if candidate.lane is lane
            ]
            lines.append(
                f"\n**{lane_labels[lane]}** ({len(lane_visible)}/{slot_limits[lane]})"
            )
            if not lane_visible:
                lines.append("- 비어 있음")
                continue
    ...
    def _format_attention_candidate(
        ...
        lines.append(
            f"   - 관찰: {self._format_direction_observation(candidate, context)}"
        )
        if context:
            label = (
                "보조근거(n/a)"
    ...
    def _format_market_status(
        ...
        mood = (
            "강세"
            if gainers > losers * 1.2
            else "약세"
            if losers > gainers * 1.2
            else "보합"
        )
    ...
    def _format_leading_sectors(
        ...
        all_changes_1h = [
            t.price_change_1h
            for t in enriched_tickers.values()
            if t.price_change_1h is not None
        ]
        all_changes_4h = [
            t.price_change_4h
            for t in enriched_tickers.values()
            if t.price_change_4h is not None
        ]
        ...
        for name, coins in SECTORS.items():
            # 데이터 유효성 체크
            tickers = [
                enriched_tickers.get(c) for c in coins if enriched_tickers.get(c)
            ]
            valid_tickers = [
                t
                for t in tickers
                if t.price_change_1h is not None and t.price_change_4h is not None
            ]
            ...
            beating_market_count = sum(
                1 for t in valid_tickers if t.price_change_1h > market_avg_1h
            )
    ...
    def _format_top_10_ranking(
        ...
        for market, rank in top_10:
            ...
            rank_strs.append(f"{rank}. {market.removeprefix('KRW-')}{change_str}")
