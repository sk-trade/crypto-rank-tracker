...
from common.models import ScanDecision, ScanEvent, ScanOutcome
...
def _row(index, score, outcome):
    ...
    event = ScanEvent(event_id=str(index), observed_at=timestamp, market="KRW-A", feature_snapshot={}, candidate_eligible=True, rejection_reasons=[], final_decision=ScanDecision.ALERT_SELECTED, model_version="test", signal_score=score)
...
def test_threshold_selection_never_uses_realized_return_to_break_score_ties():
    rows = [_row(0, 0.9, -1.0), _row(1, 0.9, 1.0)]

    selected = select_threshold(
        [event for event, _ in rows],
        [outcome for _, outcome in rows],
        1,
    )

    assert selected.net_expected_return == -1.0
